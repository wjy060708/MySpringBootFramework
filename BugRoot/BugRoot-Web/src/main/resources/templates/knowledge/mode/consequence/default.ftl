	...
	<body>
	    ...
	    <div id="mainLayout" class="easyui-layout hidden" data-options="fit: true" style="margin: 5px;padding: 5px;">
			<div data-options="region: 'center', border: false">
				<table id="dataGrid" title="缺陷后果管理" toolbar="#toolbar"></table>
		...
		<script type="text/javascript">
			...
			$(function() {
				$("#dataGrid").datagrid({
					...
					url : '${request.contextPath}/knowledge/mode/consequence/listPage.htm',
					queryParams: {
				...
				$('#barNew').click(function() {
					$.easyui.showDialog({
	                    ...
	                    href:'${request.contextPath}/knowledge/mode/consequence/new.htm',
	                    ...
	                    onSave: function (d) {
	                    	...
	                    	if(validate) {
	                    		d.form(
	                    			'submit', {
	                    				url: '${request.contextPath}/knowledge/mode/consequence/save.jsn',
				...
				$('#barEdit').click(function() {
					...
					if(dataRow != null) {
						var consequenceId = dataRow['consequenceId'];
						EditObjectEntity(consequenceId);
					}else{
						$.easyui.messager.alert('提示','请选择需要编辑的缺陷后果!','info');
					}	
				});
				...
				$('#barDelete').click(function() {
					...
					if (dataRows.length > 0) {
						$.easyui.messager.confirm('确认对话框', '确定删除选中的缺陷后果吗？', function(r) {
							if (r) {
								DeleteObjectEntity(dataRows);
							}
						});
					}else{
						$.easyui.messager.alert('提示','请选择需要删除的缺陷后果!','info');
		            	return;	
					}	
				});
			...
			function EditObjectEntity(consequenceId) {
				$.easyui.showDialog({
					title: '编辑缺陷后果',
                    ...
                    href:'${request.contextPath}/knowledge/mode/consequence/edit.htm',
                    queryParams: {
                    ...
                    onSave: function (d) {
                    	...
                    	if(validate) {
                    		d.form(
                    			'submit', {
                    				url: '${request.contextPath}/knowledge/mode/consequence/update.jsn',
                    				success: function(response) {
    									if(response) {
    										$.easyui.messager.alert('提示', response.message, 'info');
    										if(response.success) {
			...
			function DeleteObjectEntity(dataRows) {
	            ...
	            $.each(dataRows, function(i, dataRow) {
	            	consequenceIds.push(dataRow['consequenceId']);
	            });
		            
				$.ajax({
					type : 'post',
					url : '${request.contextPath}/knowledge/mode/consequence/delete.jsn',
					data : {
