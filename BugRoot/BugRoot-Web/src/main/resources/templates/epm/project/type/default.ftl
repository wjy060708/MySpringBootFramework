		...
		<script type="text/javascript">
			...
			$(function() {
				$("#dataGrid").datagrid({
					...
					url : '${request.contextPath}/epm/project/type/listPage.htm',
				...
				$('#barNew').click(function() {
					$.easyui.showDialog({
	                    ...
	                    href:'${request.contextPath}/epm/project/type/new.htm',
	                    ...
	                    onSave: function (d) {
	                    	...
	                    	if(validate) {
	                    		d.form(
	                    			'submit', {
	                    				url: '${request.contextPath}/epm/project/type/save.jsn',
				...
				$('#barEdit').click(function() {
					...
					if(dataRow != null) {
						...
						$.easyui.messager.alert('提示','请选择需要编辑的项目类型!','info');
					}	
				});
				...
				$('#barDelete').click(function() {
					...
					if (dataRows.length > 0) {
						$.easyui.messager.confirm('确认对话框', '确定删除选中的项目类型吗？', function(r) {
							if (r) {
								DeleteObjectEntity(dataRows);
							}
						});
					}else{
						$.easyui.messager.alert('提示','请选择需要删除的项目类型!','info');
		            	return;	
					}	
				});
				
				$('#barSearch').click(function() {
					var typeName = $("#txtTypeName").textbox('getValue');
			...
			function EditObjectEntity(typeId) {
				$.easyui.showDialog({
                    ...
                    href:'${request.contextPath}/epm/project/type/edit.htm',
                    queryParams: {
                    	typeId: typeId
				    },
                    ...
                    onSave: function (d) {
                    	...
                    	if(validate) {
                    		d.form(
                    			'submit', {
                    				url: '${request.contextPath}/epm/project/type/update.jsn',
                    				success: function(response) {
    									if(response) {
    										$.easyui.messager.alert('提示', response.message, 'info');
    										if(response.success) {
			...
			function DeleteObjectEntity(dataRows) {
	            ...
	            $.each(dataRows, function(i, dataRow) {
	            	typeIds.push(dataRow['typeId']);
	            });
		            
				$.ajax({
					type : 'post',
					url : '${request.contextPath}/epm/project/type/delete.jsn',
					data : {
						typeIds : typeIds
					},
					dataType: 'json',
