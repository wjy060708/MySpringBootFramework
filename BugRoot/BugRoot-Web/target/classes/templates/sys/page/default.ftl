			...
			<div data-options="region: 'center', border: false" style="padding: 5px;">
				...
				<div class="toolbar" id="toolbar">
					<div class="search-div">
						<label>页面名称:</label>
						<input id="txtPageName" class="easyui-textbox" type="text" />
						<label>功能模块:</label>
						<input id="cbModuleId" class="easyui-combobox" data-options="
							editable: false, 
							...
							loadFilter: function(data) {
								data.insert(0, {'moduleId': '', 'moduleName': '全部页面'});
								return data;
							}
							"
						/>
		...
		<script type="text/javascript">
			...
			$(function() {
				$("#dataGrid").treegrid({
					...
					pagination: true,
					pageSize: 150,
					pageList: [100, 150, 300, 500],
					
					loadMsg: '加载数据中,请稍后...',
					
					url : '${request.contextPath}/sys/page/listPage.htm',
				...
				$('#barNew').click(function() {
					$.easyui.showDialog({
	                    ...
	                    href:'${request.contextPath}/sys/page/new.htm',
	                    ...
	                    onSave: function (d) {
	                    	...
	                    	if(validate) {
	                    		d.form(
	                    			'submit', {
	                    				url: '${request.contextPath}/sys/page/save.jsn',
				...
				$('#barEdit').click(function() {
					...
					if(dataRow != null) {
						var pageId = dataRow['pageId'];
						EditObjectEntity(pageId);
					} else {
						$.easyui.messager.alert('提示','请选择需要编辑的功能页面!','info');
					}	
				});
				...
				$('#barDelete').click(function() {
					...
					if(dataRow != null) {
						var pageId = dataRow['pageId'];
						$.easyui.messager.confirm('确认对话框', '确定删除选中的功能页面吗？', function(r) {
							if (r) {
								DeleteObjectEntity(pageId);
							}
						});
						
					} else {
						$.easyui.messager.alert('提示','请选择需要删除的功能页面!','info');
					}
				});
				
				$('#barSearch').click(function() {
					var moduleId = $("#cbModuleId").combobox('getValue');
					var pageName = $("#txtPageName").textbox('getValue');
			...
			function EditObjectEntity(pageId) {
				$.easyui.showDialog({
					title: '编辑功能页面',
                    ...
                    href:'${request.contextPath}/sys/page/edit.htm',
                    queryParams: {
				    	pageId: pageId
				    },
                    ...
                    onSave: function (d) {
                    	...
                    	if(validate) {
                    		d.form(
                    			'submit', {
                    				url: '${request.contextPath}/sys/page/update.jsn',
                    				success: function(response) {
    									if(response) {
    										$.easyui.messager.alert('提示', response.message, 'info');	
    										if(response.success) {
			...
			function DeleteObjectEntity(pageId) {
				if(pageId) {
					var url = '${request.contextPath}/sys/page/delete.jsn';
					$.get(url, {pageId: pageId}, function(response) {
						if(response) {	// 成功取消
							$.easyui.messager.alert('提示', response.message, 'info');
							if(response.success) {
								$("#dataGrid").treegrid("reload");
