	...
	<body>
	    ...
	    <div id="mainLayout" class="easyui-layout hidden" data-options="fit: true">
			<div data-options="region: 'center', border: false" style="padding: 5px;">
				<table id="dataGrid" title="数据字典类别管理" toolbar="#toolbar"></table>
			
				<div class="toolbar" id="toolbar">
					<div class="search-div">
						<label>类别名称:</label>
						<input id="txtCategoryName" class="easyui-textbox" type="text" />
		...
		<script type="text/javascript">
			...
			$(function() {
				$("#dataGrid").treegrid({
					...
					url : '${request.contextPath}/sys/dict/category/listPage.htm',
				...
				$('#barNew').click(function() {
					$.easyui.showDialog({
	                    ...
	                    href:'${request.contextPath}/sys/dict/category/new.htm',
	                    
	                    onSave: function (d) {
	                    	var validate = d.form("validate");
	                    	if(validate) {
	                    		d.form('submit', {
                    				url: '${request.contextPath}/sys/dict/category/save.jsn',
				...
				$('#barEdit').click(function() {
					...
					if(dataRow != null) {
						...
						$.easyui.messager.alert('提示','请选择需要编辑的数据字典类别!','info');
					}	
				});
				...
				$('#barDelete').click(function() {
					...
					if(dataRow != null) {
						var categoryId = dataRow['categoryId'];
						$.easyui.messager.confirm('确认对话框', '确定删除选中的数据字典类别吗？', function(r) {
							if (r) {
								DeleteObjectEntity(categoryId);
							}
						});
						
					} else {
						$.easyui.messager.alert('提示','请选择需要删除的数据字典类别!','info');
					}
				});
				
				$('#barSearch').click(function() {
					var categoryName = $("#txtCategoryName").textbox('getValue');
			...
			function EditObjectEntity(categoryId) {
				$.easyui.showDialog({
                    ...
                    href:'${request.contextPath}/sys/dict/category/edit.htm',
                    queryParams: {
                    ...
                    onSave: function (d) {
                    	...
                    	if(validate) {
                    		d.form('submit', {
                				url: '${request.contextPath}/sys/dict/category/update.jsn',
                				success: function(response) {
									if(response) {
										$.easyui.messager.alert('提示', response.message, 'info');	
										if(response.success) {
			...
			function DeleteObjectEntity(categoryId) {
				if(categoryId) {
					var url = '${request.contextPath}/sys/dict/category/delete.jsn';
					$.get(url, {categoryId: categoryId}, function(response) {
						if(response) {	// 成功取消
							$.easyui.messager.alert('提示', response.message, 'info');
							if(response.success) {
								$("#dataGrid").treegrid("reload");
