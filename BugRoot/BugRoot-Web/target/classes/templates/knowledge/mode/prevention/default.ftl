	...
	<body>
	    ...
	    <div id="mainLayout" class="easyui-layout hidden" data-options="fit: true" style="margin: 5px;padding: 5px;">
			<div data-options="region: 'center', border: false">
				<table id="dataGrid" title="缺陷预防管理" toolbar="#toolbar"></table>
		...
		<script type="text/javascript">
			...
			$(function() {
				...
				$('#barNew').click(function() {
					$.easyui.showDialog({
	                    ...
	                    href:'${request.contextPath}/knowledge/mode/prevention/new.htm',
	                    ...
	                    onSave: function (d) {
	                    	...
	                    	if(validate) {
	                    		d.form(
	                    			'submit', {
	                    				url: '${request.contextPath}/knowledge/mode/prevention/save.jsn',
				...
				$('#barEdit').click(function() {
					...
					if(dataRow != null) {
						var preventionId = dataRow['preventionId'];
						EditObjectEntity(preventionId);
					}else{
						$.easyui.messager.alert('提示','请选择需要编辑的缺陷预防!','info');
					}	
				});
				...
				$('#barDelete').click(function() {
					...
					if (dataRows.length > 0) {
						$.easyui.messager.confirm('确认对话框', '确定删除选中的缺陷预防吗？', function(r) {
							if (r) {
								DeleteObjectEntity(dataRows);
							}
						});
					}else{
						$.easyui.messager.alert('提示','请选择需要删除的缺陷预防!','info');
		            	return;	
					}	
				});
			...
			function EditObjectEntity(preventionId) {
				$.easyui.showDialog({
					title: '编辑缺陷预防',
                    ...
                    href:'${request.contextPath}/knowledge/mode/prevention/edit.htm',
                    queryParams: {
                    ...
                    onSave: function (d) {
                    	...
                    	if(validate) {
                    		d.form(
                    			'submit', {
                    				url: '${request.contextPath}/knowledge/mode/prevention/update.jsn',
                    				success: function(response) {
    									if(response) {
    										$.easyui.messager.alert('提示', response.message, 'info');
    										if(response.success) {
			...
			function DeleteObjectEntity(dataRows) {
	            ...
	            $.each(dataRows, function(i, dataRow) {
	            	preventionIds.push(dataRow['preventionId']);
	            });
		            
				$.ajax({
					type : 'post',
					url : '${request.contextPath}/knowledge/mode/prevention/delete.jsn',
					data : {
						preventionIds : preventionIds
					},
					dataType: 'json',
					traditional: true,
