	...
	<head>
		...
		<script type="text/javascript">
			...
			function onDocumentReady() {
				var achieId = "${epmProjectAchie.achieId}";
				var url = '${request.contextPath}/epm/project/achie/documentReady.jsn';
				
				$.get(url, {achieId: achieId}, function(response) {
					if(response.success) {
						$.easyui.loaded($('#documnentViewer'));
						
						loadDocument(achieId); // 加载文档
					} else {
						$.util.delay(onDocumentReady, 10);
					}
				}, 'json');
			...
			function loadDocument(achieId) {
				this.location = '${request.contextPath}/epm/project/achie/getPdfDocument/' + achieId + ".pdf";
			}
		</script>
	</head>
